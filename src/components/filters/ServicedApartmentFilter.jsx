...
import { 
  ...
  ChevronLeft, ChevronRight, Utensils, Dumbbell, ParkingMeter,
  Tv, Microwave, Warehouse, Church, School, Hospital, 
  Train, ShoppingBag, Plane, Landmark, Briefcase
} from 'lucide-react';
...
const ServicedApartmentFilter = () => {
  ...
  // Common Property Details Section
  const PropertyDetailsSection = () => (
    <>
      ...
      <SelectInput label="Number of Bedrooms" options={['Studio', '1 BHK', '2 BHK', '3 BHK']} />
  ...
  const InteriorDetailsSection = () => (
    <>
      <SelectInput label="Furnishing" options={['Fully Furnished', 'Semi-Furnished']} />
      <div className="space-y-0.5 sm:space-y-1">
        {['Kitchenette / Modular Kitchen', 'Air Conditioning', 'Wi-Fi', 'Television', 'Washing Machine', 'Housekeeping Included', 'Laundry Service'].map(item => (
      ...
      <InputField label="Appliances Included" placeholder="e.g., Refrigerator, Microwave, Coffee Machine" />
    </>
  );
  ...
  const AmenitiesSection = () => (
    <div className="space-y-0.5 sm:space-y-1">
      {['Covered Parking', 'Lift', '24/7 Security', 'CCTV Surveillance', 'Power Backup', 'Gym', 'Swimming Pool', 'Reception / Front Desk', 'Restaurant / Café', 'Business Center / Conference Room'].map(amenity => (
  ...
  const NearbyAccessSection = () => (
    <div className="space-y-0.5 sm:space-y-1">
      {['IT Park / Business Hub', 'School', 'Hospital', 'Metro / Bus Stop', 'Shopping Mall / Market', 'Airport Access'].map(place => (
  ...
  const ContactPreferenceSection = () => (
    <>
      <SelectInput label="Contact via" options={['Owner', 'Agent', 'Builder', 'Operator']} />
  ...
  const renderRentFilters = () => (
    <>
      ...
      <FilterSection title="💰 Rent Details">
        ...
        <RadioGroup label="Maintenance / Service Charges Included" options={['Yes', 'No']} />
      ...
      <FilterSection title="👥 Tenant Preferences">
        <CheckboxGroup label="Tenant Type" options={['Family', 'Bachelor', 'Working Professionals', 'Corporate Stay', 'Students']} />
  ...
  const renderLeaseFilters = () => (
    <>
      ...
      <FilterSection title="📄 Lease Details">
        ...
        <SelectInput label="Lease Duration" options={['Short-term', '1 Year', '2+ Years']} />
        <RadioGroup label="Maintenance / Service Charges Included" options={['Yes', 'No']} />
      ...
      <FilterSection title="🪑 Interior Details"><InteriorDetailsSection /></FilterSection>
      <FilterSection title="👥 Tenant Preferences">
        <CheckboxGroup label="Tenant Type" options={['Family', 'Bachelor', 'Working Professionals', 'Corporate Lease', 'Students']} />
        <RadioGroup label="Pet Friendly" options={['Yes', 'No']} />
        <RadioGroup label="Dietary Preference" options={['Veg Only', 'No Restriction']} />
        <RadioGroup label="Smoking Allowed" options={['Yes', 'No']} />
      </FilterSection>
        ...
        <div className="px-3 py-1.5 sm:px-6 sm:py-2">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="p-1 sm:p-1.5 bg-white/20 rounded-lg sm:rounded-xl shadow-lg">
              <style>{`@keyframes slowSpin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }.slow-spin { animation: slowSpin 4s linear infinite; }`}</style>
              <Building className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white slow-spin" />
            </div>
            <h1 className="text-sm sm:text-base md:text-lg font-bold text-white">Serviced Apartment Filters</h1>
          </div>
        </div>
      </div>
...
export default ServicedApartmentFilter;