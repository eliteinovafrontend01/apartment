...
import { 
  ...
  Gauge, Key, Lock, LayoutTemplate
  // Removed Stairs from here since it doesn't exist
} from 'lucide-react';

// Custom Stairs icon component (replacing the missing Stairs from lucide-react)
const Stairs = ({ className, ...props }) => (
  <svg
...
const DuplexApartmentFilter = () => {
  ...
  const PropertyDetailsSection = ({ showOwnership = false, showHomeOffice = false }) => (
    <>
      ...
      <SelectInput label="Number of Bedrooms" options={['2 BHK', '3 BHK', '4 BHK+']} />
      <SelectInput label="Number of Bathrooms" options={['1', '2', '3', '4+']} />
      <NumberInputField label="Number of Floors (Duplex Levels)" placeholder="Enter number of levels" />
      <NumberInputField label="Floor Number in Building" placeholder="Enter floor number" />
      <SelectInput label="Facing Direction" options={['North', 'South', 'East', 'West', 'North-East', 'North-West', 'South-East', 'South-West']} />
      <SelectInput label="Balcony / Terrace" options={['Balcony', 'Terrace', 'Both', 'None']} />
  ...
  const InteriorDetailsSection = ({ showHomeOffice = false, showAppliances = true }) => (
    <>
      <SelectInput label="Furnishing" options={['Fully Furnished', 'Semi-Furnished', 'Unfurnished']} />
      <div className="space-y-0.5 sm:space-y-1">
        {['Modular Kitchen', 'Air Conditioning', 'Wardrobes', 'Utility Area', 'Private Staircase'].map(item => (
        ...
        {showHomeOffice && (
          <label className="flex items-center gap-1 sm:gap-1.5 cursor-pointer group">
            <input type="checkbox" className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded border-2 border-teal-300 text-teal-600 focus:ring-teal-500 focus:ring-offset-0 accent-teal-600" />
            <span className="text-xs sm:text-sm text-teal-700 group-hover:text-teal-600 transition-colors font-medium">Home Office Space</span>
          </label>
        )}
      </div>
      {showAppliances && (
  ...
  // Amenities Section
  const AmenitiesSection = ({ showSmartFeatures = false }) => (
    <>
      <div className="space-y-0.5 sm:space-y-1">
        {['Covered Parking', 'Lift', '24/7 Security', 'CCTV Surveillance', 'Power Backup', 'Gym', 'Swimming Pool', 'Clubhouse', 'Garden / Open Space', "Children's Play Area"].map(amenity => (
      ...
      {showSmartFeatures && (
        <div className="mt-2">
          <CheckboxGroup label="Smart Features" options={['Smart Home Features']} />
        </div>
      )}
    </>
  );
  ...
  const renderRentFilters = () => (
    <>
      <FilterSection title="📋 Basic Details">
        <InputField label="Property Type" placeholder="Duplex Apartment" type="text" />
      ...
      <FilterSection title="🪑 Interior Details"><InteriorDetailsSection showHomeOffice={false} showAppliances={true} /></FilterSection>
      <FilterSection title="👥 Tenant Preferences">
        <CheckboxGroup label="Tenant Type" options={['Family', 'Bachelor', 'Working Professionals']} />
      ...
      <FilterSection title="✨ Amenities"><AmenitiesSection showSmartFeatures={false} /></FilterSection>
  ...
  const renderBuyFilters = () => (
    <>
      ...
      <FilterSection title="🪑 Interior Details"><InteriorDetailsSection showHomeOffice={true} showAppliances={true} /></FilterSection>
      <FilterSection title="✨ Amenities"><AmenitiesSection showSmartFeatures={true} /></FilterSection>
      <FilterSection title="📅 Availability">
  ...
  return (
    <div className="h-screen flex flex-col bg-emerald-50 rounded-3xl" style={{ overflow: 'hidden' }}>
      {/* Sticky Header with Stairs/Duplex Icon */}
        ...
        <div className="px-3 py-1.5 sm:px-6 sm:py-2">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="p-1 sm:p-1.5 bg-white/20 rounded-lg sm:rounded-xl shadow-lg">
              ...
              `}</style>
              <Stairs className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white slow-spin" />
            </div>
            <h1 className="text-sm sm:text-base md:text-lg font-bold text-white">Duplex Apartment Filters</h1>
          </div>
        </div>
      </div>
...
export default DuplexApartmentFilter;