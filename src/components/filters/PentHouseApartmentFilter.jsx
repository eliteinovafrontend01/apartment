...
import { 
  ...
  Gauge, Key, Lock, LayoutTemplate, Gem, Building2, Crown
} from 'lucide-react';
...
const PenthouseApartmentFilter = () => {
  ...
  const PropertyDetailsSection = ({ showOwnership = false }) => (
    <>
      ...
      <SelectInput label="Number of Bedrooms" options={['1 BHK', '2 BHK', '3 BHK', '4 BHK', '5 BHK+']} />
      <SelectInput label="Number of Bathrooms" options={['1', '2', '3', '4', '5+']} />
      <NumberInputField label="Floor Number (Top Floor)" placeholder="Enter top floor number" />
      ...
      <SelectInput label="Private Terrace / Rooftop" options={['Private Terrace', 'Rooftop', 'Both', 'None']} />
      <SelectInput label="View Type" options={['Sea View', 'Lake View', 'Garden View', 'City View', 'Mountain View', 'No View']} />
  ...
  const InteriorDetailsSection = ({ showAppliances = false }) => (
    <>
      ...
      <div className="space-y-0.5 sm:space-y-1">
        {['Modular Kitchen', 'Walk-in Closet', 'Air Conditioning', 'Smart Home Features', 'Private Elevator Access', 'Home Office Space'].map(item => (
      ...
      {showAppliances && (
        <InputField label="Appliances Included" placeholder="e.g., Refrigerator, AC, Washing Machine, Microwave, Dishwasher" />
      )}
    </>
  );

  // Luxury Amenities Section
  const LuxuryAmenitiesSection = ({ showPetFriendly = false, showConcierge = true }) => (
    <>
      <div className="space-y-0.5 sm:space-y-1">
        {['Covered Parking / Valet Parking', 'Lift', '24/7 Security', 'CCTV Surveillance', 'Power Backup', 'Swimming Pool', 'Gym', 'Clubhouse', 'Jacuzzi / Spa', 'Rooftop Lounge', 'Concierge Service'].map(amenity => (
  ...
  const renderRentFilters = () => (
    <>
      <FilterSection title="📋 Basic Details">
        <InputField label="Property Type" placeholder="Penthouse Apartment" type="text" />
      ...
      <FilterSection title="✨ Luxury Amenities"><LuxuryAmenitiesSection showPetFriendly={false} /></FilterSection>
  ...
  const renderBuyFilters = () => (
    <>
      ...
      <FilterSection title="🪑 Interior Details"><InteriorDetailsSection showAppliances={true} /></FilterSection>
      <FilterSection title="✨ Luxury Amenities"><LuxuryAmenitiesSection showPetFriendly={true} /></FilterSection>
      <FilterSection title="📅 Availability">
  ...
  const renderLeaseFilters = () => (
    <>
      ...
      <FilterSection title="📄 Lease Details">
        ...
        <SelectInput label="Lease Duration" options={['1 Year', '2 Years', '3+ Years']} />
        <RadioGroup label="Maintenance Charges Included" options={['Yes', 'No']} />
      ...
      <FilterSection title="🪑 Interior Details"><InteriorDetailsSection showAppliances={true} /></FilterSection>
      <FilterSection title="👥 Tenant Preferences">
        <CheckboxGroup label="Tenant Type" options={['Family', 'Working Professionals', 'Corporate Lease']} />
        <RadioGroup label="Pet Friendly" options={['Yes', 'No']} />
        <RadioGroup label="Dietary Preference" options={['Veg Only', 'No Restriction']} />
        <RadioGroup label="Smoking Allowed" options={['Yes', 'No']} />
      </FilterSection>
  ...
  return (
    <div className="h-screen flex flex-col bg-emerald-50 rounded-3xl" style={{ overflow: 'hidden' }}>
      {/* Sticky Header with Crown/Penthouse Icon */}
        ...
        <div className="px-3 py-1.5 sm:px-6 sm:py-2">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="p-1 sm:p-1.5 bg-white/20 rounded-lg sm:rounded-xl shadow-lg">
              ...
              `}</style>
              <Crown className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white slow-spin" />
            </div>
            <h1 className="text-sm sm:text-base md:text-lg font-bold text-white">Penthouse Apartment Filters</h1>
          </div>
        </div>
      </div>
