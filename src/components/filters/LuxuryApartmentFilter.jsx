...
const LuxuryApartmentFilter = () => {
  ...
  const PropertyDetailsSection = ({ showOwnership = false }) => (
    <>
      ...
      <SelectInput label="Number of Bedrooms" options={['2 BHK', '3 BHK', '4 BHK', '5 BHK+']} />
      ...
      <SelectInput label="Balcony / Private Terrace" options={['Balcony', 'Private Terrace', 'Both', 'None']} />
      <SelectInput label="View Type" options={['Sea View', 'Lake View', 'Garden View', 'City Skyline View', 'No View']} />
  ...
  const InteriorDetailsSection = ({ showAppliances = true }) => (
    <>
      ...
      <div className="space-y-0.5 sm:space-y-1">
        {['Modular Kitchen', 'Walk-in Closet', 'Home Office Space', 'Air Conditioning', 'Smart Home Automation', 'Premium Interiors'].map(item => (
  ...
  const LuxuryAmenitiesSection = ({ showPetFriendly = false, showWifi = false }) => (
    <>
      <div className="space-y-0.5 sm:space-y-1">
        {['Covered Parking / Valet Parking', 'Lift', '24/7 Security', 'CCTV Surveillance', 'Power Backup', 'Infinity Swimming Pool', 'Gym / Fitness Center', 'Spa / Sauna / Jacuzzi', 'Clubhouse', 'Rooftop Lounge', 'Concierge Service', 'Banquet Hall', 'Tennis / Sports Court', 'Children\'s Play Area', 'Private Garden / Sky Deck'].map(amenity => (
  ...
  const NearbyAccessSection = () => (
    <div className="space-y-0.5 sm:space-y-1">
      {['School', 'Hospital', 'Metro / Bus Stop', 'Shopping Mall / Luxury Retail', 'IT Park / Business Hub', 'Airport Access'].map(place => (
  ...
  const renderRentFilters = () => (
    <>
      <FilterSection title="📋 Basic Details">
        <InputField label="Property Type" placeholder="Luxury Apartment" type="text" />
      ...
      <FilterSection title="👥 Tenant Preferences">
        <CheckboxGroup label="Tenant Type" options={['Family', 'Working Professionals', 'Corporate Stay']} />
        <RadioGroup label="Pet Friendly" options={['Yes', 'No']} />
        <RadioGroup label="Dietary Preference" options={['Veg Only', 'No Restriction']} />
        <RadioGroup label="Smoking Allowed" options={['Yes', 'No']} />
      </FilterSection>
      <FilterSection title="✨ Luxury Amenities"><LuxuryAmenitiesSection showPetFriendly={false} showWifi={true} /></FilterSection>
  ...
  const renderBuyFilters = () => (
    <>
      ...
      <FilterSection title="🪑 Interior Details"><InteriorDetailsSection showAppliances={true} /></FilterSection>
      <FilterSection title="✨ Luxury Amenities"><LuxuryAmenitiesSection showPetFriendly={true} showWifi={false} /></FilterSection>
      <FilterSection title="📅 Availability">
...
export default LuxuryApartmentFilter;