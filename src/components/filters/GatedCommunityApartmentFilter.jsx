...
// ── Main Component ──────────────────────────────────────────────────────────
const GatedCommunityApartmentFilter = () => {
  ...
  const PropertyDetailsSection = ({ showOwnership = true }) => (
    <>
  ...
  const CommunityAmenitiesSection = () => (
    <div className="space-y-0.5 sm:space-y-1">
      {['Covered Parking', 'Lift', '24/7 Security', 'CCTV Surveillance', 'Power Backup', 'Swimming Pool', 'Gym', 'Clubhouse', "Children's Play Area", 'Garden / Park', 'Jogging Track', 'Indoor Games', 'Community Hall', 'Visitor Parking', 'Pet Friendly'].map(amenity => (
  ...
  const ContactPreferenceSection = ({ includeBuilder = true }) => (
    <>
  ...
  const renderRentFilters = () => (
    <>
      <FilterSection title="📋 Basic Details">
        <InputField label="Property Type" placeholder="Gated Community Apartment" type="text" />
      ...
      <FilterSection title="🏘️ Community Amenities">
        <CommunityAmenitiesSection />
        <div className="mt-2">
  ...
  const renderBuyFilters = () => (
    <>
      ...
      <FilterSection title="🏘️ Community Amenities"><CommunityAmenitiesSection /></FilterSection>
      <FilterSection title="📅 Availability">
  ...
  return (
    <div className="h-screen flex flex-col bg-emerald-50 rounded-3xl" style={{ overflow: 'hidden' }}>
      {/* Sticky Header with Gate/Security Icon */}
        ...
        <div className="px-3 py-1.5 sm:px-6 sm:py-2">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="p-1 sm:p-1.5 bg-white/20 rounded-lg sm:rounded-xl shadow-lg">
              ...
              `}</style>
              <Shield className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white slow-spin" />
            </div>
            <h1 className="text-sm sm:text-base md:text-lg font-bold text-white">Gated Community Apartment Filters</h1>
          </div>
        </div>
      </div>
...
export default GatedCommunityApartmentFilter;